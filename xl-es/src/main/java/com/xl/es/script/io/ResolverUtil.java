...
public class ResolverUtil<T> {
	...
	/**
	 * Recursively list all resources under the given URL that appear to define
	 * a Java class. Matching resources will have a name that ends in ".class"
	 * and have a relative path such that each segment of the path is a valid
	 * Java identifier. The resource paths returned will be relative to the URL
	 * and begin with the specified path.
	 * 
	 * @param url
	 *            The URL of the parent resource to search.
	 * @param path
	 *            The path with which each matching resource path must begin,
	 *            relative to the URL.
	 ...
	 * begin with the specified {@code path}. Entries will match with or
	 ...
	 * matters in the search for classes. Relevant resources would be class
	 * files themselves (file names that end with ".class") and directories that
	 * might be a Java package name segment (java identifiers).
	 * 
	 * @param resourceName
